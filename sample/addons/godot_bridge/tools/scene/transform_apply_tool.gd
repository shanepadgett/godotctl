...
func execute(args: Dictionary) -> Dictionary:
	...
	var config_result := _parse_config(str(args.get("value_json", "")), "value_json")
	...
	if raw_values.is_empty():
		return _finish(root, _result.error(_errors.INVALID_ARGS, "value_json must include at least one transform key"))

	var property_names := _sorted_property_names(raw_values)
	var decoded_values: Dictionary = {}
	for property_name in property_names:
		if property_name.is_empty() or not _TRANSFORM_KEYS.has(property_name):
			return _finish(root, _result.error(_errors.INVALID_ARGS, "value_json contains unsupported key: %s" % property_name))
	...
	return _finish(root, _result.success("node transform applied: %s" % node_path, {
